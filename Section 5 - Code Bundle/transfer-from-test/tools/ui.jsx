...
const GlobalStyles = createGlobalStyle`
  ...
  :root {
    ...
    --fg-default: #F5F9FC;
		...
		--bg-higher: #2B3245;
		--bg-highest: #3C445C;
		--outline-default: #AF49FF;
		--outline-dimmer:  #EB6FFF;
		--outline-dimmest: #4E5569;
		--overlay: #0e1525A0;

		/**accents */
		--accent-primary-dimmer: #EB6FFF;
		--accent-primary-default: #AF49FF;
...
export default function App() {
  ...
  React.useEffect(() => {
    const callMoralis = async () => {
      let userEthNFTs = await Moralis.Web3API.account.getNFTs(walletAddress);
  ...
  if (typeof window.ethereum === "undefined") {
    return (
      ...
      >
        <h1 style={{ textAlign: "center" }}>Replit 🤝 Polygon 🤝 Pointer </h1>
        <a
...
function Balance({ walletAddress, chainId }) {
  ...
  return (
    <span style={{ marginRight: "var(--space-8)", whiteSpace: "nowrap" }}>
      {ethers.utils.formatEther(balance).slice(0, 6)} MATIC
    </span>
  );
}
  ...
  return (
    <VStack style={{ alignItems: "start" }}>
      <HStack style={{ alignItems: "center" }}>
        <Dot style={{ marginRight: "var(--space-8)" }} color="#EB6FFF" />
        <span
          style={{
            color:
              chainId === 1
          ...
          <HelpIndicator
            text={
              chainId === 1
                ...
                : "You are connected to a test network. Test networks let you deploy your contracts with fake ETH/MATIC"
            }
          />
        </span>
      </HStack>
      {chainId === 1 && (
        ...
        >
          Switch to the Polygon Mumbai test network via MetaMask 🦊
        </span>
      )}
    </VStack>
  );
